...
self.addEventListener('install', e => {
  self.skipWaiting();
  e.waitUntil(caches.open(VER).then(c => c.addAll(['./','./index.html','./manifest.json',...CDN])));
});
...
self.addEventListener('fetch', e => {
  ...
  // Toujours réseau en premier pour index.html
  if (url.pathname.endsWith('index.html')||url.pathname==='/') {
    e.respondWith(fetch(e.request).then(r=>{
      const c=r.clone(); caches.open(VER).then(ca=>ca.put(e.request,c)); return r;
